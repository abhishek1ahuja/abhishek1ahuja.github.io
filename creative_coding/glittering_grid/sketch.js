...
function mousePressed(){
  if(mouseX > (width - 200) && mouseY > (height - 60)){

  }
  else{
    mousecolor = get(mouseX, mouseY);
    initr = red(mousecolor);
    initg = green(mousecolor);
    initb = blue(mousecolor);
  }
}
...
class Tile{
  ...
  update(){
    ...
    if(dummy > threshold){
        this.r = initr + int(random(-d,d));
        this.g = initg + int(random(-d,d));
        this.b = initb + int(random(-d,d));
    }
  }
  display(){
